...
if model is not None and observable_scaler is not None and target_scaler is not None:
    if target_column not in resampled_data.columns or timestamp_column not in resampled_data.columns:
        ...
        try:
            ...
            if merged.empty:
                st.write("Aún no hay datos reales coincidentes con las marcas de tiempo predichas. Espera a que lleguen datos reales.")
            else:
                # Verificar que existan las columnas para comparar
                if target_column in merged.columns and f"{target_column}_pred" in merged.columns:
                    # Calcular error solo si hay datos
                    if len(merged) > 0:
                        ...
                        if len(merged["error"]) > 0:
                            ...
                            if len(r) > 0:
                                ...
                                st.write("No hay datos suficientes para graficar el bullseye chart.")
                        else:
                            st.write("No hay datos suficientes para calcular el error.")
                    else:
                        st.write("No hay registros para comparar.")
                else:
                    st.write("No se encuentran columnas reales y predichas para comparar. Espera más datos reales.")
        except Exception as e:
